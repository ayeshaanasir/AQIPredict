...
def load_predictions():
    """Load latest predictions from MongoDB"""
    try:
        db = get_database()
...
def main():
    ...
    with st.sidebar:
        ...
        st.header("🎨 AQI Scale")
        st.markdown("""
        <div style='background-color: #00e400; padding: 5px; margin: 2px; border-radius: 3px;'>
        <b>1 - Good</b>
        </div>
        <div style='background-color: #ffff00; padding: 5px; margin: 2px; border-radius: 3px;'>
        <b>2 - Fair</b>
        </div>
        <div style='background-color: #ff7e00; padding: 5px; margin: 2px; border-radius: 3px; color: white;'>
        <b>3 - Moderate</b>
        </div>
        <div style='background-color: #ff0000; padding: 5px; margin: 2px; border-radius: 3px; color: white;'>
        <b>4 - Poor</b>
        </div>
        <div style='background-color: #8f3f97; padding: 5px; margin: 2px; border-radius: 3px; color: white;'>
        <b>5 - Very Poor</b>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        max_72h = predictions_df['predicted_aqi'].max()
        st.metric("72h Peak", int(max_72h))
    
    # Health Advisory
    st.info(f"**Health Advisory:** {description}")
    
    # AQI Health Information
    with st.expander("ℹ️ AQI Health Guidelines & Recommendations"):
        ...
        | 2 | Fair | Acceptable for most people | Unusually sensitive people should consider limiting prolonged outdoor exertion |
        | 3 | Moderate | Sensitive groups may experience effects | Active children and adults, and people with respiratory disease should limit prolonged outdoor exertion |
        | 4 | Poor | Everyone may begin to experience effects | Active children and adults, and people with respiratory disease should avoid prolonged outdoor exertion |
        | 5 | Very Poor | Health alert: everyone may experience serious effects | Everyone should avoid all outdoor exertion |
        
        ### Who is at Risk?
        - Children and teenagers
        - Older adults
        - People with lung diseases (asthma, COPD)
    ...
    with tab1:
        ...
        fig.add_hline(y=2.5, line_dash="dash", line_color="yellow",
        ...
        if len(predictions_df) >= 24:
            ...
            hourly_df['category'] = hourly_df['predicted_aqi'].apply(
                lambda x: get_aqi_category(x)[0]
            )
            hourly_df['color'] = hourly_df['predicted_aqi'].apply(
    ...
    with tab2:
        ...
        if not historical_df.empty:
            ...
            fig3.update_layout(
                title="Historical AQI (Last 7 Days)",
                xaxis_title="Date",
                yaxis_title="AQI Value",
                hovermode='x unified',
                height=400
            ...
            with col5:
                ...
                mode_display = int(mode_val[0]) if len(mode_val) > 0 else "N/A"
                st.metric("Most Common", mode_display)
            ...
            st.info("No historical data available. Run the feature pipeline to collect data.")
        ...
        if not predictions_df.empty:
            ...
            with col1:
                ...
                fig5 = px.scatter(
                    ...
                    y='predicted_aqi',
                    color='predicted_aqi',
                    title="Temperature vs Predicted AQI",
            ...
            with col2:
                ...
                fig6 = px.scatter(
                    ...
                    y='predicted_aqi',
                    color='predicted_aqi',
                    title="Humidity vs Predicted AQI",
            ...
            # Wind speed
            st.subheader("Wind Speed Forecast (48 Hours)")
            fig7 = px.line(
                predictions_df.head(48),
                x='timestamp',
                y='windspeed',
            ...
            current_weather = predictions_df.iloc[0]
            ...
            with col3:
                st.metric("💨 Wind Speed", f"{current_weather['windspeed']:.1f} m/s")
            with col4:
        ...
        if not predictions_df.empty:
            ...
            with col2:
                if not historical_df.empty:
                    ...
                    hist_category_counts = historical_df['aqi'].apply(
                        lambda x: get_aqi_category(x)[0]
                    ).value_counts()
                    
                    fig10 = px.pie(
                        values=hist_category_counts.values,
                        names=hist_category_counts.index,
                        title="AQI Category Distribution (7 days)",
                        color=hist_category_counts.index,
                        color_discrete_map={
                            'Good': '#00e400',
                            'Fair': '#ffff00',
                            'Moderate': '#ff7e00',
