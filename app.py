...
def show_aqi_alerts(predictions_df: pd.DataFrame) -> None:
    """Display prominent alert banners when predicted AQI exceeds safe thresholds."""
    if predictions_df.empty or "predicted_aqi" not in predictions_df.columns:
        return

    max_aqi = int(predictions_df["predicted_aqi"].max())
...
def main():
    ...
    with st.sidebar:
        ...
        for bg, fg, label in aqi_scale_info:
            st.markdown(
    ...
    with col3:
        ...
        metric_card("24h Average", f"{avg_24h:.0f}" if avg_24h else "N/A")
    with col4:
    ...
    with st.expander("ℹ️ US EPA AQI Health Guidelines"):
        st.markdown("""
        | AQI Range | Category | Who is Affected | Recommendation |
        |-----------|----------|-----------------|----------------|
        | 0–50 | Good | No one | Enjoy outdoor activities |
        | 51–100 | Moderate | Very sensitive people | Consider reducing prolonged outdoor exertion |
        | 101–150 | Unhealthy for Sensitive Groups | Sensitive groups | Limit prolonged outdoor exertion |
        | 151–200 | Unhealthy | Everyone | Reduce prolonged or heavy outdoor exertion |
        | 201–300 | Very Unhealthy | Everyone | Avoid prolonged outdoor exertion |
        | 301–500 | Hazardous | Everyone | Stay indoors; keep windows closed |
        """)
    ...
    with tab1:
        ...
        # Reference lines at US EPA threshold boundaries
        ...
        fig.update_layout(
            yaxis=dict(range=[0, max(500, int(predictions_df["predicted_aqi"].max()) + 20)]),
            yaxis_title="AQI (US EPA 0-500)",
            hovermode="x unified",
        )
        ...
        for _, row in daily.iterrows():
            ...
            _, day_color, _ = get_aqi_category(row["Max AQI"])
            with c1:
                st.markdown(
    ...
    with tab2:
        st.subheader("Historical AQI Trends (Last 7 Days) — US EPA scale")
    ...
    with tab3:
        ...
        with c1:
            fig5 = px.scatter(
                predictions_df, x="temperature", y="predicted_aqi",
        ...
        with c2:
            fig6 = px.scatter(
                predictions_df, x="humidity", y="predicted_aqi",
        ...
        fig7 = px.line(
            predictions_df.head(48), x="timestamp", y="windspeed",
            title="Wind Speed Forecast (48h)", labels={"windspeed": "Wind Speed (m/s)"},
        )
    ...
    with tab4:
        ...
        if "aqi_category" not in ddf.columns:
            ddf["aqi_category"] = ddf["predicted_aqi"].apply(lambda x: get_aqi_category(x)[0])
    ...
    with tab5:
        ...
        hp = (
            predictions_df.groupby("hour")["predicted_aqi"]
            .agg(["mean", "std"])
            .reset_index()
        )
        fig8 = go.Figure()
        ...
        with c1:
            cc = predictions_df["predicted_aqi"].apply(
                lambda x: get_aqi_category(x)[0]
            ).value_counts()
            fig9 = px.pie(
                values=cc.values, names=cc.index,
        ...
        with c2:
            if not historical_df.empty and "aqi" in historical_df.columns:
                hc = historical_df["aqi"].apply(
                    lambda x: get_aqi_category(x)[0]
                ).value_counts()
                fig10 = px.pie(
                    values=hc.values, names=hc.index,
        ...
        # SHAP feature importance (if plot exists)
        ...
        if os.path.exists(shap_path):
            ...
            st.info("SHAP importance plot not yet generated. It will appear here after training.")
